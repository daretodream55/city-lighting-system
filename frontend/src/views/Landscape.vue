<template>
  <div>
    ...
    <div class="search-bar">
      ...
      <el-upload :show-file-list="false" :before-upload="handleImport" accept=".xlsx,.xls" action=""><el-button icon="el-icon-upload2">导入</el-button></el-upload>
    </div>
    <div class="table-container">
      <el-table :data="list" stripe v-loading="loading" size="small">
        ...
        <el-table-column prop="name" label="名称" min-width="160" /><el-table-column prop="district_name" label="区划" width="90" />
        <el-table-column prop="road_section" label="路段" width="130" /><el-table-column prop="scene_type" label="场景类型" width="100" />
        <el-table-column prop="light_count" label="灯具数量" width="90" align="right" /><el-table-column prop="power_load" label="功率(KW)" width="90" align="right" />
        <el-table-column prop="control_mode" label="控制方式" width="100" />
        <el-table-column prop="status" label="状态" width="80">
          <template slot-scope="s"><el-tag :type="s.row.status==='normal'?'success':s.row.status==='fault'?'danger':'warning'" size="small">{{ s.row.status==='normal'?'正常':s.row.status==='fault'?'故障':'待维护' }}</el-tag></template>
        </el-table-column>
    ...
    <el-dialog :title="isEdit?'编辑':'新增'" :visible.sync="dialogVisible" width="650px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-row :gutter="20">
          ...
          <el-col :span="12"><el-form-item label="场景类型"><el-input v-model="form.scene_type" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="控制方式"><el-input v-model="form.control_mode" /></el-form-item></el-col>
...
import api from '../api'
export default {
  name: 'Landscape', data() { return { list:[],total:0,loading:false,districts:[],search:{keyword:'',district_id:'',status:'',page:1,pageSize:20},dialogVisible:false,isEdit:false,saving:false,form:{},rules:{box_code:[{required:true,message:'表箱编码不能为空'}],name:[{required:true,message:'名称不能为空'}]},templateUrl:'/api/templates/landscape-import' }},
  mounted() { this.loadDistricts(); this.loadData() },
  methods: {
    async loadDistricts() { const r=await api.get('/system/districts'); this.districts=r||[] },
    async loadData() { this.loading=true; try{const r=await api.get('/landscape',{params:this.search}); this.list=r.list;this.total=r.total}catch(e){}finally{this.loading=false} },
    openDialog(row){ this.isEdit=!!row;this.form=row?{...row}:{box_code:'',name:'',district_id:null,road_section:'',location:'',longitude:null,latitude:null,scene_type:'',light_count:0,power_load:0,control_mode:'',install_date:'',status:'normal',remark:''};this.dialogVisible=true },
    async save(){ this.$refs.formRef.validate(async v=>{if(!v)return;this.saving=true;try{if(this.isEdit)await api.put('/landscape/'+this.form.id,this.form);else await api.post('/landscape',this.form);this.$message.success(this.isEdit?'更新成功':'新增成功');this.dialogVisible=false;this.loadData()}catch(e){}finally{this.saving=false}}) },
    async del(id){ await api.delete('/landscape/'+id);this.$message.success('删除成功');this.loadData() },
    async handleImport(f){ const fd=new FormData();fd.append('file',f);await api.post('/landscape/import/excel',fd,{headers:{'Content-Type':'multipart/form-data'}});this.loadData();return false }
  }
}
</script>
